...
static long time_timeout_compare(const void* a, const void* b) {
  struct time_timeout* const x = (struct time_timeout*) a;
  struct time_timeout* const y = (struct time_timeout*) b;
  if(x->time > y->time) {
    return 1;
...
static long time_interval_compare(const void* a, const void* b) {
  struct time_interval* const x = (struct time_interval*) a;
  struct time_interval* const y = (struct time_interval*) b;
...
int time_manager(struct time_manager* const manager, void (*on_timeout_expire)(struct time_manager*, const struct time_timeout*), void (*on_interval_expire)(struct time_manager*, const struct time_interval*), const unsigned long timeouts_alloc_size, const unsigned long intervals_alloc_size) {
  *manager = (struct time_manager) {
    ...
    .on_timeout_expire = on_timeout_expire,
    .on_interval_expire = on_interval_expire
  };
  ...
  if(!refheap_is_empty(&manager->timeouts)) {
    if(!refheap_is_empty(&manager->intervals)) {
      ...
      if(timeout->time > time) {
        ...
        atomic_store(&manager->latest, timeout->time & 18446744073709551614UL);
      }
    } else {
      atomic_store(&manager->latest, ((struct time_timeout*) refheap_peak(&manager->timeouts, 0))->time & 18446744073709551614UL);
    }
  } else {
    if(!refheap_is_empty(&manager->intervals)) {
      struct time_interval* const interval = refheap_peak(&manager->intervals, 0);
