...
static int (*_pthread_create)(pthread_t*, const pthread_attr_t*, void* (*)(void*), void*) = NULL;
int pthread_create(pthread_t* a, const pthread_attr_t* b, void* (*c)(void*), void* d) {
  ...
  if(preemption_off) {
    queue[queue_len++] = (struct preemption_data) {
      .a = a,
      .b = b,
      .c = c,
      .d = d
...
void test_preempt_on(void) {
  ...
  for(int i = 0; i < queue_len; ++i) {
    assert(!_pthread_create(queue[i].a, queue[i].b, queue[i].c, queue[i].d));
  }
  queue_len = 0;
}
