...
struct tcp_socket* reject_only(struct tcp_socket* serv, struct tcp_socket* sock, enum tcp_event event) {
	switch(event) {
		case tcp_close: {
			tcp_socket_free(serv);
			break;
		}
		case tcp_free: {
			test_mutex_wake();
...
struct tcp_socket* return_self_only(struct tcp_socket* serv, struct tcp_socket* sock, enum tcp_event event) {
	switch(event) {
		case tcp_open: {
			switch(server_onevt) {
				case 0: {
...
main()
{
	...
	assert(tcp_socket(servers, NULL));
	test_end();

	test_begin("tcp server err 2");
	struct tcp_socket_options serv_options = {0};
	assert(tcp_socket(servers, &serv_options));
	test_end();
	...
	char port[7] = {0};
	assert(sprintf(port, "%hu", tcp_socket_get_port(servers)) > 0);
